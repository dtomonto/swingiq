...
export function FeatureDetail({ initial }: { initial: DetailData }) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
      <Link href="/admin/feature-education" className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-amber-400">
      ...
      <SectionCard title="Coverage" description="The learning assets this feature warrants, and where each one stands.">
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
          {warranted.map((t) => {
            ...
            return (
              <div key={t} className="rounded-lg border border-gray-800 p-2.5">
                <div className="flex items-center justify-between gap-2">
                  <span className="text-xs font-medium text-gray-300">{ASSET_TYPE_LABELS[t]}</span>
                  {status ? (
      ...
      <SectionCard title={`Generated assets (${initial.assets.length})`} description="Review, approve and publish. Nothing is public until you publish it.">
        {initial.assets.length === 0 ? (
          <p className="text-sm text-gray-500">No assets yet — click <strong>Generate package</strong> to draft everything this feature warrants.</p>
        ) : (
      ...
      <HelpPanel>
        <p>
          <strong className="text-gray-300">Generate</strong> drafts every asset this feature warrants, each grounded in
          the real surfaces above, then quality-scored and security-scanned. <strong className="text-gray-300">Approve</strong>{' '}
          a draft once it reads well, then <strong className="text-gray-300">Publish</strong> it. Video scripts publish
...
function Meta({ label, values }: { label: string; values: string[] }) {
  return (
    <div className="flex gap-2">
      ...
      <dd className="min-w-0 text-gray-300">{values.length ? values.join(', ') : <span className="text-gray-600">—</span>}</dd>
    </div>
  );
}
    ...
    <div className="rounded-lg border border-gray-800">
      <div className="flex flex-wrap items-center justify-between gap-2 p-3">
        <button onClick={() => setOpen((o) => !o)} className="flex min-w-0 items-center gap-2 text-left">
          ...
          <span className="min-w-0">
            <span className="block truncate text-sm font-medium text-gray-100">{ASSET_TYPE_LABELS[asset.type]}</span>
            <span className="block truncate text-xs text-gray-500">{asset.title}</span>
          </span>
        </button>
      ...
      {open && (
        <div className="space-y-3 border-t border-gray-800 p-3">
          <p className="text-sm text-gray-400">{asset.summary}</p>

          {asset.sections.map((s, i) => (
            <div key={i}>
              <h4 className="text-xs font-semibold uppercase tracking-wide text-gray-500">{s.heading}</h4>
          ...
          {asset.faqs && asset.faqs.length > 0 && (
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-wide text-gray-500">Q&amp;A</h4>
              <ul className="mt-1 space-y-1.5 text-sm">
                {asset.faqs.map((f, i) => (
                  <li key={i}><span className="text-gray-200">{f.q}</span><br /><span className="text-gray-400">{f.a}</span></li>
                ))}
              </ul>
            </div>
          )}
          ...
          {asset.seo && (
            <div className="rounded bg-gray-950 p-2 text-xs text-gray-400">
              <p><span className="text-gray-500">SEO title:</span> {asset.seo.title}</p>
              <p><span className="text-gray-500">Slug:</span> /{asset.seo.slug}</p>
              <p><span className="text-gray-500">Schema:</span> {asset.seo.schema.join(', ')}</p>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            <button onClick={onPublish} disabled={!canPublish || busy !== null} className="inline-flex items-center gap-1 rounded-lg bg-amber-500 px-2.5 py-1 text-xs font-semibold text-gray-950 hover:bg-amber-400 disabled:opacity-40">
              <Upload className="h-3.5 w-3.5" /> Publish
            </button>
            <button onClick={onReject} disabled={busy !== null} className="inline-flex items-center gap-1 rounded-lg border border-gray-700 px-2.5 py-1 text-xs font-medium text-gray-300 hover:bg-gray-800 disabled:opacity-40">
              <Send className="h-3.5 w-3.5" /> Send back
            </button>
            <button onClick={onRegenerate} disabled={busy !== null} className="inline-flex items-center gap-1 rounded-lg border border-gray-700 px-2.5 py-1 text-xs font-medium text-gray-300 hover:bg-gray-800 disabled:opacity-40">
