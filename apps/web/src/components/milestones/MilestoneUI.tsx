...
const STATUS_TONE: Record<MilestoneStatus, string> = {
  ...
  needs_data_source: 'bg-muted/40 text-muted-foreground border-border/50',
...
export function ProgressBar({ pct }: { pct: number | null }) {
  if (pct === null) return <span className="text-xs text-muted-foreground">No data source</span>;
...
function Pill({ children, className, title }: { children: ReactNode; className: string; title?: string }) {
  return (
    <span title={title} className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-semibold ${className}`}>
      {children}
    </span>
  );
}
