...
export function ChallengeView({ challenge }: { challenge: Challenge }) {
  return (
    <main className="min-h-screen bg-white">
      <div className="bg-[#1a3a2a] px-4 py-12 text-white">
        <div className="mx-auto max-w-2xl">
          ...
          <p className="mt-3 text-green-100">{challenge.intro}</p>
        </div>
      </div>

      <div className="mx-auto max-w-2xl px-4 py-10">
        <div className="rounded-2xl border border-green-200 bg-green-50 p-4">
          <p className="text-sm text-green-900"><strong>The promise:</strong> {challenge.promise}</p>
        </div>
        ...
        <ol className="mt-8 space-y-4">
          {challenge.days.map((d) => (
            <li key={d.day} className="flex items-start gap-4 rounded-xl border border-gray-200 p-4">
              ...
              <div>
                <p className="font-semibold text-gray-900">{d.focus}</p>
                <p className="mt-0.5 text-sm text-gray-600">{d.detail}</p>
              </div>
            </li>
          ))}
        </ol>
        ...
        <p className="mt-3 text-xs italic text-gray-500">{challenge.safetyNote}</p>
        ...
        <div className="mt-6 rounded-2xl bg-[#1a3a2a] p-6 text-center text-white">
          <p className="mb-4 text-lg font-bold">Track your progress with a free analysis</p>
          <Link href="/dashboard" className="inline-block rounded-xl bg-green-500 px-8 py-3 font-bold text-white transition-colors hover:bg-green-400">
