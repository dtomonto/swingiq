...
export function GlobalSearch({ open, onClose, can }: GlobalSearchProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-[12vh]">
      <button aria-label="Dismiss search" className="absolute inset-0 bg-foreground/60" onClick={onClose} />
            ...
            return (
              <li key={`nav-${item.id}`}>
                <button
                  ...
                  disabled={!item.built}
                  className={`flex w-full items-center gap-3 px-3 py-2 text-left text-sm disabled:opacity-50 ${i === active ? 'bg-muted' : ''}`}
                >
                  {showRecent ? <Clock className="h-4 w-4 shrink-0 text-muted-foreground" /> : <Icon className="h-4 w-4 shrink-0 text-link" />}
            ...
            return (
              <li key={`ent-${entity.type}-${entity.id}`}>
                <button
                  ...
                  className={`flex w-full items-center gap-3 px-3 py-2 text-left text-sm ${idx === active ? 'bg-muted' : ''}`}
                >
                  <Icon className="h-4 w-4 shrink-0 text-link" />
                  <span className="min-w-0 flex-1">
                    <span className="truncate text-foreground">{entity.label}</span>
                    {entity.sublabel && <span className="block truncate text-xs text-muted-foreground">{entity.sublabel}</span>}
                  </span>
                  {idx === active && <CornerDownLeft className="h-3 w-3 text-muted-foreground/70" />}
                </button>
              </li>
            );
          })}
