  ...
  if (total === 0) {
    return (
      <div className="rounded-xl border border-dashed border-gray-800 bg-gray-900/50 p-8 text-center">
        <p className="text-sm text-gray-300">No open recommendations. 🎉</p>
        <p className="mt-1 text-xs text-gray-500">Every open finding has been triaged or accepted. Re-run the scan to recompute from the latest signals.</p>
      </div>
    );
  }
        ...
        return (
          <section key={b}>
            <h2 className="mb-2 flex items-center gap-2 text-sm font-semibold text-gray-200">
              <Icon className="h-4 w-4 text-amber-400" /> {BUCKET_LABEL[b]}
...
function RecCard({ rec }: { rec: SecurityRecommendation }) {
  ...
  const bandTone =
    rec.priorityBand === 'critical' ? 'text-red-300' : rec.priorityBand === 'high' ? 'text-orange-300' : rec.priorityBand === 'medium' ? 'text-amber-300' : 'text-sky-300';
  return (
    <li className="rounded-xl border border-gray-800 bg-gray-900">
      <button onClick={() => setOpen((o) => !o)} className="flex w-full items-start justify-between gap-3 p-4 text-left">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <span className="text-sm font-bold tabular-nums text-gray-400">{rec.priorityScore}</span>
          <ChevronDown className={`h-4 w-4 text-gray-600 transition-transform ${open ? 'rotate-180' : ''}`} />
        </div>
      </button>
      {open && (
        <div className="space-y-3 border-t border-gray-800 p-4 pt-3">
          <Field label="Why it matters">{rec.whyItMatters}</Field>
          <Field label="What could happen">{rec.whatCouldHappen}</Field>
          <div>
            <p className="mb-1 text-[11px] font-medium uppercase tracking-wide text-gray-500">Steps</p>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {rec.relatedLinks.map((l) => (
              <Link key={l.href} href={l.href} className="rounded-lg border border-gray-700 px-2 py-1 text-xs text-gray-300 hover:bg-gray-800">{l.label}</Link>
            ))}
            {rec.canClaudeFix && (
              <span className="inline-flex items-center gap-1 text-[11px] text-emerald-300"><Bot className="h-3 w-3" /> Claude Code can implement this now</span>
            )}
          </div>
        </div>
      )}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="mb-0.5 text-[11px] font-medium uppercase tracking-wide text-gray-500">{label}</p>
