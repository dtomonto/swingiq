...
export function BenchmarksClient() {
  ...
  if (!table) return <p className="text-sm text-gray-400">Loading…</p>;
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={save} disabled={!dirty} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-sm font-semibold text-white disabled:opacity-50">
          <Save size={14} /> {savedAt ? 'Saved' : 'Save override'}
        </button>
        <button onClick={reset} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-600 px-3 py-1.5 text-sm font-medium text-gray-200 hover:bg-gray-700">
          <RotateCcw size={14} /> Reset to defaults
        </button>
        <button onClick={copy} className="inline-flex items-center gap-1.5 rounded-lg border border-gray-600 px-3 py-1.5 text-sm font-medium text-gray-200 hover:bg-gray-700">
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="border-b border-gray-700 bg-gray-800/50">
              <th className="px-3 py-2 text-left font-semibold text-gray-300">Profile</th>
              {GRADE_DIMENSIONS.map((d) => (
                <th key={d} className="px-2 py-2 text-center font-medium text-gray-400">{DIMENSION_LABELS[d]}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {GOLF_PROFILES.map((p) => (
              <tr key={p.id} className="border-b border-gray-800 last:border-0">
                <td className="px-3 py-2">
                  ...
                  <span className="ml-1 text-xs text-gray-500">hcp {p.handicapRange[0]}–{p.handicapRange[1]}</span>
                </td>
                {GRADE_DIMENSIONS.map((d) => (
                  <td key={d} className="px-2 py-1.5 text-center">
                    <input
                      ...
                      className="w-14 rounded border border-gray-600 bg-gray-900 px-1.5 py-1 text-center text-gray-100"
                    />
                  </td>
                ))}
              </tr>
