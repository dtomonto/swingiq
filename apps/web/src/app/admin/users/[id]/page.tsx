  ...
  if (!detail.user) {
    return (
      <div className="mx-auto max-w-4xl space-y-4 p-4 sm:p-6">
        {back}
        <SectionCard>
          <p className="py-6 text-center text-sm text-muted-foreground">User not found.</p>
        </SectionCard>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-4xl space-y-6 p-4 sm:p-6">
        ...
        <SectionCard title="Sport profiles" description="Non-golf profiles this athlete has set up.">
          <div className="space-y-4">
            {detail.sportProfiles.map((sp) => (
              <div key={sp.sport}>
                <p className="mb-1 text-sm font-medium text-link">{sportLabel(sp.sport)}</p>
      ...
      <SectionCard title="Recent sessions" description={`${detail.sessions.length} shown (latest 50).`}>
        {detail.sessions.length === 0 ? (
          <p className="text-sm text-muted-foreground">No practice sessions recorded.</p>
        ) : (
              ...
              <tbody className="text-foreground">
                {detail.sessions.map((s) => (
                  <tr key={s.id} className="border-t border-border">
                    <td className="max-w-[12rem] truncate py-2 pr-3 text-foreground">{s.name || '—'}</td>
                    <td className="py-2 pr-3">{sportLabel(s.sport)}</td>
                    <td className="py-2 pr-3 text-muted-foreground">{s.date || (s.created_at ? formatDate(s.created_at) : '—')}</td>
      ...
      <SectionCard title="Recent analyses" description="Metadata only — videos are processed privately and never stored.">
        {detail.analyses.length === 0 ? (
          <p className="text-sm text-muted-foreground">No swing analyses recorded.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="text-[11px] uppercase tracking-wide text-muted-foreground">
              ...
              <tbody className="text-foreground">
                {detail.analyses.map((a) => (
                  <tr key={a.id} className="border-t border-border">
                    ...
                    <td className="max-w-[10rem] truncate py-2 pr-3 text-muted-foreground">{a.file_name || '—'}</td>
                    <td className="py-2 pr-3 tabular-nums">{a.overall_score ?? '—'}</td>
                    <td className="max-w-[12rem] truncate py-2 pr-3 text-muted-foreground">{a.primary_issue ?? '—'}</td>
      ...
      <HelpPanel>
        <p>
          <strong className="text-foreground">What this is.</strong> Everything SwingVantage knows about this
          account in one place — profile, sessions, analyses and engagement — so you can understand their
          journey at a glance.
        </p>
        <p>
          <strong className="text-foreground">Actions.</strong> <em>Export data</em> downloads a JSON copy of
