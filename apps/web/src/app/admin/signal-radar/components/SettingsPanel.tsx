...
import { RotateCcw, Save, Play, Copy, Rss } from 'lucide-react';
...
function ScheduledCollection({ config, onUpdate }: {
  config: SignalRadarConfig;
  onUpdate: (patch: Partial<SignalRadarConfig>) => void;
}) {
  ...
  return (
    <SectionCard
      title={<span className="flex items-center gap-2"><Rss className="h-4 w-4 text-muted-foreground" /> Scheduled collection</span>}
      description="Keyless, ToS-safe automated collection. Add RSS/Atom feed URLs (a blog /feed, reddit.com/r/golf/search.rss?q=swing+analysis, a YouTube channel feed). A scheduler hitting GET /api/signal-radar/cron with CRON_SECRET (or this Run now) fetches + classifies them. Persists only when the durable store is configured."
      actions={
        <div className="flex gap-2">
