    ...
    <div className="space-y-4">
      ...
      <SectionCard title="Signal clusters" description="Recurring themes — patterns worth a page, FAQ, feature or response.">
        {dashboard.clusters.length === 0 ? (
          <p className="text-xs text-gray-600">No repeated themes yet. Clusters appear once two or more signals share an intent + sport.</p>
        ) : (
          <ul className="space-y-2">
            {dashboard.clusters.map((cl) => (
              <li key={cl.id} className="flex items-center justify-between gap-3 rounded-lg border border-gray-800 bg-gray-950/40 px-3 py-2">
                <button onClick={() => cl.signalIds[0] && onOpenSignal(cl.signalIds[0])} className="min-w-0 flex-1 text-left text-sm text-gray-200 hover:text-amber-300">
                  {cl.theme}
                </button>
