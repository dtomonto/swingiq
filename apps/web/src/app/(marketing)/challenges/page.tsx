    ...
    <main className="min-h-screen bg-card">
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {list.map((c) => (
            <Link key={c.slug} href={`/challenges/${c.slug}`} className="block rounded-2xl border border-border p-5 transition-colors hover:border-primary/50 hover:bg-primary/10">
              <p className="text-xs font-semibold uppercase tracking-wide text-primary">{c.sport}</p>
              <h2 className="mt-1 font-semibold text-foreground">{c.title}</h2>
              <p className="mt-1 text-sm text-muted-foreground">{c.promise}</p>
            </Link>
          ))}
        </div>
      </div>
