...
const SECTIONS = [
  ...
  },
  {
    ...
    terms: [
      ...
      { term: 'Deterministic Intelligence', def: 'Analysis that follows explicit, written rules, so the same inputs always produce the same answer and you can trace why. SwingVantage leads with deterministic intelligence — repeatable and explainable — before using AI selectively. See /deterministic-intelligence.' },
      ...
      { term: 'Heuristic Data', def: 'Information read through encoded expert rules of thumb to surface the most likely pattern quickly and consistently — a confident estimate, not a guess and not a guaranteed measurement. See /resources/what-is-heuristic-data.' },
      { term: 'Heuristic Engine', def: 'The deterministic, rules-based part of SwingVantage that runs first on every analysis. It composes curated fault knowledge and a drill library into a normalized result with a confidence score — no external AI call required.' },
      { term: 'Rules Engine', def: 'The configuration-driven core that checks your shot data and symptoms against sport-specific target windows to identify the best-fitting pattern. Because the rules are explicit, the result is repeatable and inspectable.' },
      { term: 'Recommendation Engine', def: 'The step that ranks possible fixes by impact and confidence so you get one high-leverage priority fix — not twenty competing tips. Rankings are driven by the deterministic diagnosis and your profile.' },
      { term: 'Evidence Label', def: 'A tag that separates what SwingVantage measured (a sensor or launch-monitor number) from what it estimated (an inferred likely pattern), so an estimate is never presented as a lab measurement.' },
      { term: 'Retest', def: 'A short follow-up protocol attached to each recommendation — how many shots, which metrics, and what counts as success. Retest outcomes confirm whether a fix worked and inform future, more confident recommendations.' },
      { term: 'Confidence Score', def: 'A 0–100 calibration of how much to trust a finding, scaled by sample size, shot-to-shot consistency, and how complete your inputs were. More consistent evidence raises the score.' },
