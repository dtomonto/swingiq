...
import { useSwingVantageStore, type BaseballBat } from '@/store';
...
export function BaseballEquipment() {
  const { sportEquipment, addBaseballBat, removeBaseballBat } = useSwingVantageStore();
    ...
    <>
      <div className="p-6 max-w-2xl mx-auto space-y-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-foreground">⚾ Baseball Bat</h1>
        ...
        <div className="flex gap-3 bg-accent-secondary/10 border border-accent-secondary/25 rounded-xl p-4">
          ...
          <p className="text-sm text-foreground">
            <strong>Optional.</strong> You can skip this and still use all SwingVantage features. Missing specs lower confidence scores — they don&apos;t block anything.
          </p>
        </div>
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      ...
                      {bat.drop && <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">Drop {bat.drop}</span>}
                      {bat.certification && <span className="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">{bat.certification}</span>}
                  ...
                  <div className="space-y-3 pt-2 border-t border-border text-sm">
                    ...
                    {confirmDelete === bat.id ? (
                      <div className="flex gap-2 pt-1">
                        ...
                        <Button size="sm" className="bg-error text-error-foreground hover:bg-error/90" onClick={() => { removeBaseballBat(bat.id); setConfirmDelete(null); }}>Delete</Button>
                      </div>
                    ) : (
            ...
            <CardBody className="space-y-4">
              ...
              {form.material === 'composite' && (
                <div className="flex items-center gap-3">
                  <input type="checkbox" id="bi" checked={form.composite_broken_in === true} onChange={(e) => set('composite_broken_in', e.target.checked)} className="w-4 h-4 rounded-sm text-error" />
        ...
        {bats.length === 0 && !showForm && (
          <div className="flex gap-2 text-sm text-muted-foreground bg-muted border border-border rounded-xl p-4">
            ...
            No bat logged. This is optional — you can still use all SwingVantage features without it.
          </div>
        )}
      </div>
    </>
