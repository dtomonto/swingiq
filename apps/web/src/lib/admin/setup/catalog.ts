...
export const CATALOG: SetupTask[] = [
  ...
  // ── Growth & money ─────────────────────────────────────────
  {
    ...
    steps: [
      ...
      'Optional add-on (free heatmaps + session replay): create a project at clarity.microsoft.com, copy the Project ID from Settings → Overview, and set NEXT_PUBLIC_CLARITY_PROJECT_ID. It records sessions & sets cookies, so pair with a cookie banner in the EU. It can run alongside any of the above. To read Clarity numbers inside /admin/clarity, also add CLARITY_DATA_EXPORT_TOKEN (Clarity → Settings → Data export) — server-side, no NEXT_PUBLIC_ prefix.',
    ...
    inputs: [
      ...
      { kind: 'env', value: 'NEXT_PUBLIC_CLARITY_PROJECT_ID', example: 'abcd1234ef (publishable)', where: 'Clarity → Settings → Overview' },
      { kind: 'env', value: 'CLARITY_DATA_EXPORT_TOKEN', secret: true, example: 'eyJ… (server-side; powers /admin/clarity live metrics)', where: 'Clarity → Settings → Data export' },
