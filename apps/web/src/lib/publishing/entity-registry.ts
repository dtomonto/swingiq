...
export const PUBLISHABLE_AREAS: PublishableArea[] = [
  ...
  area({
    ...
    owner: 'Content',
    recommendedAction: 'None — /dev-updates (list + detail) merges the durable override, mirroring /updates.',
  }),
  area({
    key: 'seo-page',
    ...
    liveConnected: true,
    adminHref: '/admin/seo',
    publicRoutes: ['/sitemap.xml', '/seo/*'],
    owner: 'SEO',
    recommendedAction: 'Publish STATE controls the CRAWL surface — a durable override drops/adds the page in the sitemap (de-index/index). The static page render is source-controlled (deploy-backed) by design, so hard route removal stays a deploy.',
  }),
  area({
    key: 'blog-post',
    area: 'Blog posts',
    ...
    owner: 'Content',
    recommendedAction: 'Publish STATE is live (/blog list + post merge the durable override); NEW posts still publish deploy-backed (source-controlled).',
  }),
  area({
    key: 'milestone',
