...
import { isMinor, shareUrl, type CoachViewSnapshot } from './share';
import { SPORT_META as SPORT_META_REF } from './sports';
...
/**
 * Snapshot-based packet for the public coach view — self-contained so a coach
 * on another device can print without the athlete's full store.
 */
export function buildPacketHtmlFromSnapshot(snap: CoachViewSnapshot): string {
  const a = snap.athlete;
  const sport = snap.sport;
  const sportName = SPORT_META_REF[sport].name;
  const accent = SPORT_META_REF[sport].accentColor;

  const metricRows = snap.metrics
    .filter((m) => m.currentValue != null)
    .map((m) => {
      const def = getMetricDef(m.metricKey);
      const verified = VERIFIED_SOURCES.has(m.source as never) || m.coachValidated;
      return `<tr><td>${esc(def?.label ?? m.metricKey)}</td><td class="num">${m.currentValue}${m.unit ? ' ' + esc(m.unit) : ''}</td><td><span class="src ${verified ? 'ok' : ''}">${esc(DATA_SOURCE_LABEL[m.source])}</span></td></tr>`;
    })
    .join('');

  const filmRows = snap.film
    .slice(0, 12)
    .map((f) => `<li><b>${esc(f.title)}</b> — ${esc(f.category.replace(/_/g, ' '))}${f.opponentOrEvent ? ` · ${esc(f.opponentOrEvent)}` : ''}</li>`)
    .join('');

  const contactRows = (snap.contact ?? []).map((c) => `${esc(c.label)}: ${esc(c.value)}`).join('<br/>');

  return `<!doctype html><html lang="en"><head><meta charset="utf-8" /><meta name="robots" content="noindex" />
<title>${esc(a.name)} — Recruiting Packet</title>
<style>
  :root { --ink:#0b1220; --muted:#64748b; --accent:${accent}; --line:#e2e8f0; }
  body { font-family: ui-sans-serif, system-ui, sans-serif; color: var(--ink); margin: 0; padding: 32px; }
  .head { display:flex; justify-content:space-between; border-bottom:3px solid var(--accent); padding-bottom:14px; }
  h1 { margin:0; font-size:26px; } .sub { color:var(--muted); font-size:14px; margin-top:4px; }
  .brand { text-align:right; font-weight:800; color:var(--accent); }
  h2 { font-size:13px; text-transform:uppercase; letter-spacing:.06em; color:var(--muted); margin:22px 0 8px; }
  table { width:100%; border-collapse:collapse; font-size:13px; } td { padding:6px 8px; border-bottom:1px solid var(--line); }
  .num { text-align:right; font-weight:600; } .src { font-size:11px; color:var(--muted); } .src.ok { color:#15803d; font-weight:600; }
  ul { padding-left:18px; font-size:13px; } .foot { margin-top:26px; border-top:1px solid var(--line); padding-top:12px; color:var(--muted); font-size:11px; }
</style></head><body>
  <div class="head">
    <div><h1>${esc(a.name)}</h1><div class="sub">${[a.graduationYear ? `Class of ${a.graduationYear}` : '', sportName, a.position ?? '', a.schoolOrClub ?? ''].filter(Boolean).join(' · ')}</div></div>
    <div class="brand">SwingVantage</div>
  </div>
  ${snap.summary ? `<h2>Player summary</h2><p style="font-size:14px;line-height:1.5">${esc(snap.summary.body)}</p>` : ''}
  <h2>Key metrics</h2>${metricRows ? `<table><tbody>${metricRows}</tbody></table>` : '<p class="sub">No data shared.</p>'}
  <h2>Film</h2>${filmRows ? `<ul>${filmRows}</ul>` : '<p class="sub">No film shared.</p>'}
  ${contactRows ? `<h2>Contact</h2><p style="font-size:13px">${contactRows}</p>` : ''}
  <div class="foot">${snap.disclosures.map((d) => esc(d)).join(' ')} Generated by SwingVantage.</div>
</body></html>`;
}
...
export function openPacketForPrint(state: RecruitingState, variant: PacketVariant, profileLink?: string): boolean {
  return printHtml(buildPacketHtml(state, variant, profileLink));
}

/** Coach-view packet from a snapshot (no athlete store needed). */
export function openSnapshotPacketForPrint(snap: CoachViewSnapshot): boolean {
  return printHtml(buildPacketHtmlFromSnapshot(snap));
}
